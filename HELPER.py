...
def most_successful(df, sport):
    ...
    medal_count = temp_df['Name'].value_counts().reset_index().head(15)
    medal_count.columns = ['Name', 'Medals']  # Rename the columns
...
def Tmost_successful_countrywise(df, country):
    temp_df = df.dropna(subset=['Medal'])
    ...
    # Count the number of medals won by each athlete
    medal_count = temp_df['Name'].value_counts().reset_index().head(20)
    medal_count.columns = ['Name', 'count']  # Rename the columns

    # Merge with the original DataFrame to get additional information
    x = medal_count.merge(df, left_on='Name', right_on='Name', how='left')[
        ['Name', 'count', 'Sport']].drop_duplicates('Name')

    # Rename 'count' to 'Medals' for clarity
    x.rename(columns={'count': 'Medals'}, inplace=True)

    # Assign ranks to the athletes
    x['Rank'] = x['Medals'].rank(ascending=False, method='dense').astype(int)

    # Sort by rank to ensure it's in order
    x = x.sort_values(by='Rank').reset_index(drop=True)
